...
func TestIO(t *testing.T) {
	...
	fl, err := ReadForwardListFromFile(textTestFile)
	require.Nil(t, err)
	require.Nil(t, fl.Head)
	fl, err = ReadForwardListFromFileBinary(binTestFile)
	require.Nil(t, err)
	require.Nil(t, fl.Head)
	...
	textArr, err := ReadForwardListFromFile(textTestFile)
	require.Nil(t, err)
	binArr, err := ReadForwardListFromFileBinary(binTestFile)
	require.Nil(t, err)
	...
	_, err = ReadForwardListFromFile(badPath)
	assert.Error(t, err)
	_, err = ReadForwardListFromFileBinary(badPath)
	assert.Error(t, err)
}
